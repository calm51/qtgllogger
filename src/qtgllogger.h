...
const QString &_qgl_file_full_path = QStringLiteral(__FILE__); \
    ...
    if (_qgl_file_name_length<=0){_qgl_file_name_length=0;} \
...
public:
    ...
    void blank(Level level=Level::Debug) {
        if (level < logLevel || level < globalLogLevel) {return;}
    ...
    void debug(Args&&... args) {
        if (Level::Debug < logLevel || Level::Debug < globalLogLevel) {return;}
        if(module.isEmpty()){return;}
    ...
    template <typename... Args>
    void info(Args&&... args) {
        if (Level::Info < logLevel || Level::Info < globalLogLevel) {return;}
        if(module.isEmpty()){return;}
    ...
    void warning(Args&&... args) {
        if (Level::Warning < logLevel || Level::Warning < globalLogLevel) {return;}
        if(module.isEmpty()){return;}
    ...
    void error(Args&&... args) {
        if (Level::Error < logLevel || Level::Error < globalLogLevel) {return;}
        if(module.isEmpty()){return;}
...
private:
    ...
    QDebug header(Level level, const QStringList &extinfos={}) {
        QDebug qdebug = qDebug().noquote().nospace() << "["
